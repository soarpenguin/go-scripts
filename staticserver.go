...
func upload(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		if err != nil {
			fmt.Fprintf(w, "%v", "upload error")
			return
		}
		fileext := filepath.Ext(handler.Filename)
		if check(fileext) == false {
			fmt.Fprintf(w, "%v", "type not allowed")
			return
		}
		...
		if err != nil {
			fmt.Fprintf(w, "%v", "upload failed")
			return
		}
		filedir, _ := filepath.Abs(Upload_Dir + filename)
		fmt.Fprintf(w, "%v", filename+"upload finished, server address:"+filedir)
	}
}
